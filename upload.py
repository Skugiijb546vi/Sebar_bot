...
def get_size_mb(file_path):
    """بۆ زانینی قەبارەی فایلەکە بە مێگابایت"""
    if os.path.exists(file_path):
        return os.path.getsize(file_path) / (1024 * 1024)
    return 0

def check_and_compress(file_path):
    """ئەگەر فایلەکە لە ٢ گێگا گەورەتر بوو، بچووکی دەکاتەوە"""
    ...
    if size > 1990:
        ...
        # کەمکردنەوەی کوالێتی بە رێژەیەکی کەم بۆ ئەوەی قەبارەی دابەزێت
        subprocess.run([
            "ffmpeg", "-i", file_path, 
    ...
    with app:
        ...
        if mode == "only_video":
            ...
            if os.path.exists(target_file):
                app.send_video(chat_id=chat_id, video=target_file, caption="🎬 ڤیدیۆکە ئامادەیە (قەبارە ڕێکخراوە)")
            return

        # داگرتنی ڤیدیۆ بۆ مۆدەکانی تر
